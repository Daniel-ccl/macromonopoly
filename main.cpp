...
int main() {
    InitWindow(screenWidth, screenHeight, "Monopoly - Tableros Conectados (Fortunas)");
    SetTargetFPS(60);
    ...
    while (!WindowShouldClose()) {
        ...
        // Dibujar las esferas negras en casillas Fortuna (solo decorativo)
        {
            ...
            Vector3 top1 = { p1.x, p1.y + tablero1[fortuna1_t1].altura / 2.0f + 0.15f, p1.z };
            Vector3 top2 = { p2.x, p2.y + tablero2[fortuna1_t2].altura / 2.0f + 0.15f, p2.z };
            DrawSphere(top1, 0.18f, BLACK);
            DrawSphere(top2, 0.18f, BLACK);
        }
        {
            Vector3 p1 = tablero1[fortuna2_t1].posicion;
            Vector3 p2 = tablero2[fortuna2_t2].posicion;
            Vector3 top1 = { p1.x, p1.y + tablero1[fortuna2_t1].altura / 2.0f + 0.15f, p1.z };
            Vector3 top2 = { p2.x, p2.y + tablero2[fortuna2_t2].altura / 2.0f + 0.15f, p2.z };
            DrawSphere(top1, 0.18f, BLACK);
            DrawSphere(top2, 0.18f, BLACK);
        }
